...
# ─── Chat Input/Output (User -> API)────────────────────────────────────────────────────────

class ChatMessage(BaseModel): #used for conversation history
...
class ChatRequest(BaseModel):
    ...
    session_id: str = Field(..., description="Unique session identifier") #user tracking
    message: str = Field(..., description="User's symptom description or follow-up") #current input
    conversation_history: List[ChatMessage] = Field(
        default_factory=list,
        description="Previous messages for context" #memory
    )
    pdf_text: Optional[str] = Field(
        None,
        description="Extracted text from uploaded medical PDF" # upload medical report, PDF format
    )

 
class ExtractedSymptom(BaseModel): # AI Processing Models (internal pipeline)
...
class PossibleCondition(BaseModel): # This is your diagnosis reasoning layer
...
class ReasoningStep(BaseModel): # Tracks How the AI reached its conclusion
...
class HealthcareResponse(BaseModel): # Final Output to Frontend, MAIN OUTPUT
    """
    Structured response from the multi-agent pipeline.
    This is the main output format returned to the frontend.
    """
    session_id: str #user tracking
    original_query: str #user input

    # Agent outputs
    extracted_symptoms: List[ExtractedSymptom] = Field(default_factory=list) #symptoms extracted
    possible_conditions: List[PossibleCondition] = Field(default_factory=list) #conditions identified
